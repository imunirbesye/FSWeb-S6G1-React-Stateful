...
export default function Programcilar() {
  ...
  const [oneCikanId, setOneCikan] = useState();
  ...
  const oneCikaninIsmi = () => {
    ...
    return programcilar[oneCikanId-1].isim;
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {/* burada dev.id 'yi öne çıkan id'ye atayın */ setOneCikan(dev.id); }}>Kutla</button>
            </div>
          )
        }
      </div>
